...
SCRIPTS = [
    "MTC-UAT-0001-Inicio_Sesion_Inicio_Sesion_Exitoso.py",
    "MTC-UAT-0002-Inicio_Sesion_Inicio_Sesion_Fallido_C.py",
    "MTC-UAT-0003-Inicio_Sesion_Inicio_Sesion_Fallido_C.py",
    "MTC-UAT-0005-Inicio_Sesion_Mostrar_Mis_Dispositivo.py",
    "MTC-UAT-0006-Inicio_Sesion_Filtrar_Sistema_Operati.py",
...
def _lit(valor):
    """
    Literal Python seguro para un valor. Se usa como reemplazo en las
    sustituciones del outline. json.dumps escapa comillas, backslashes y saltos
    de línea; antes se escapaba solo la comilla, así que un valor con backslash
    rompía el script o permitía cerrar el string e inyectar código.
    """
    import json as _json
    return _json.dumps(str(valor))


class _ResultadoTimeout:
    """
    Sustituto de CompletedProcess cuando un script excede el tiempo límite.

    Sin timeout, un Chromium colgado bloqueaba el runner para siempre: no se
    escribía results.json ni el reporte. Ahora se corta y se registra como
    fallo, y el resto de scripts sigue.
    """
    def __init__(self, segundos):
        self.returncode = 1
        self.stdout = ""
        self.stderr = f"El script excedió el tiempo límite de {segundos}s y fue cancelado."


# Tiempo máximo por script. Igual que el de las iteraciones de outline.
SCRIPT_TIMEOUT = 300


def _run_script(path):
    """Ejecuta un script con timeout; devuelve un resultado incluso si expira."""
    try:
        return subprocess.run(
            [sys.executable, path], capture_output=True, text=True, timeout=SCRIPT_TIMEOUT
        )
    except subprocess.TimeoutExpired:
        print(f"[TIMEOUT] {path} superó {SCRIPT_TIMEOUT}s")
        return _ResultadoTimeout(SCRIPT_TIMEOUT)
        ...
        if default_val != new_val:
            ...
            for i, line in enumerate(lines):
                if '.fill(' in line and default_val in line:
                    # El reemplazo va por función para que re.sub NO interprete
                    # backslashes del valor. El literal completo lo da _lit().
                    lines[i] = re.sub(
                        r'\.fill\(\s*["\']' + old_escaped + r'["\']',
                        lambda m: f'.fill({_lit(new_val)}',
                        line, count=1
                    )
    return '\n'.join(lines)
...
def _substitute_manual_columns(source, original_values, data_set, manual_columns):
    ...
    for col_name in manual_columns:
        ...
        reemplazo = _lit(new_value)
        # Reemplazo por función: evita que \g, \1, etc. del valor se interpreten.
        result = re.sub(r'"' + old_escaped + r'"', lambda m: reemplazo, result, count=1)
...
# Sesiones descifradas en esta corrida; se borran al final (finally).
_auth_temp_files = []
...
def _decrypt_auth(enc_path):
    """
    Descifra un .auth.enc usando HAKA_AUTH_KEY. Soporta el formato Fernet nuevo
    (HKENC2) y el XOR heredado (HKENC1). Escribe a un temporal registrado para
    borrarse al terminar: las cookies descifradas no deben quedar en el disco.
    """
    import tempfile
    ...
    try:
        ...
        plano = None
        if data.startswith(b"HKENC2"):
            try:
                import base64
                from cryptography.fernet import Fernet
                from cryptography.hazmat.primitives import hashes
                from cryptography.hazmat.primitives.kdf.pbkdf2 import PBKDF2HMAC
                salt_hex = os.environ.get("HAKA_AUTH_SALT", "")
                if not salt_hex:
                    print("[AUTH] Falta HAKA_AUTH_SALT para descifrar formato Fernet")
                    return None
                kdf = PBKDF2HMAC(algorithm=hashes.SHA256(), length=32,
                                 salt=bytes.fromhex(salt_hex), iterations=200_000)
                fkey = base64.urlsafe_b64encode(kdf.derive(key.encode()))
                plano = Fernet(fkey).decrypt(data[6:])
            except Exception as _fe:
                print(f"[AUTH] No se pudo descifrar (Fernet): {_fe}")
                return None
        elif data.startswith(b"HKENC1"):
            import hashlib
            ks = hashlib.sha256(key.encode()).digest()
            plano = bytes(b ^ ks[i % 32] for i, b in enumerate(data[6:]))
        else:
            return None

        fd, out_path = tempfile.mkstemp(suffix=".auth.json", dir=os.path.dirname(enc_path))
        with os.fdopen(fd, "wb") as f:
            f.write(plano)
        _auth_temp_files.append(out_path)

        try:
            import json as _j
            with open(out_path, "r", encoding="utf-8") as _vf:
                _parsed = _j.load(_vf)
            print(f"[AUTH] Sesion descifrada OK ({len(_parsed.get('cookies', []))} cookies)")
        except Exception as _ve:
            print(f"[AUTH] JSON descifrado INVALIDO: {_ve}")
            return None
        return out_path
...
def _limpiar_auth_temp():
    """Borra las sesiones descifradas de esta corrida."""
    for p in _auth_temp_files:
        try:
            os.unlink(p)
        except OSError:
            pass
    ...
    if step_runner_path.exists():
        try:
            ...
            if IS_CI:
                ...
                print("---")
            r = _run_script(instrumented_path)
...
# Borrar las sesiones descifradas: no deben quedar en claro en el workspace.
_limpiar_auth_temp()
